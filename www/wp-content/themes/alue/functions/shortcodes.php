		...
		$code = '
	                <div class="span4 service-span">
	                    <div class="service-box">
	                        ...
	                        <h3>'.$title.'</h3>
	                        <p>'.$text.'</p>
	                        <a class="btn" href="'.$url.'">Veja Mais</a>
	                    </div>
	                </div>					
				';
		return $code;
...
//Frase
	...
	add_shortcode('frase', 'mt_frase');

	function mt_frase($atts=array(), $content = null) {
	    ob_start();
	    frase($atts, $content);
	    $content = ob_get_clean();
	    return $content;
	...
	function frase($atts=array(), $content = null) {
		require (get_template_directory() . '/functions/m-frase.php');
	}
...
// Portfolio ShortCode
	function mt_portfolio($atts){
		...
		$project_id = get_the_ID();
		$project_title = get_the_title($project_id);
		$project_content = esc_html(do_shortcode(get_the_content($project_id)));
		$thumbnail = get_the_post_thumbnail($project_id, 'portfolio_img');
		$projects_link = get_post_meta( $project_id, '_cmb_port_link', true );
